package com.gohan.mikebamb.domain

import java.sql.Timestamp

data class EquipmentModel(
    var partNumberModel: String,
    var nameModel: String,
    var manufacturerModel: String,
    var modelModel: String,
    var installDateModel: String,
    var fluigModel: String,
    var linkToManualModel: String,
    var hoursModel: String,
    var qrCodeModel: String,
    var commentsModel: String,
    var category1Model: String,
    var category2Model: String,
    var category3Model: String,
    var observation1Model: String,
    var observation2Model: String,
    var observation3Model: String,
    var observation4Model: String,
    var observation5Model: String,
    var timestamp: Timestamp
)



